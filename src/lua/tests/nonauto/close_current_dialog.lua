...
(e.g., "Copy"/"Move") to think the user had OK'ed the action.
...
Note: A third way to close a dialog is with dlg:command("ok"). It does
'h->ret_value = B_ENTER'. Whether a built-in MC dialog expects this value
(or B_CANCEL) you'll know only by trial-and-error or by looking into its
C source code.
