...
[[Mouse handling test.]]))
  ...
  wgt.on_click = function(self, x, y, buttons, count)
    point.x = x
    point.y = y
    -- Note: after processing a key or mouse event, MC automatically re-positions the cursor. So we don't need to trigger on_cursor ourselves.
    -- Note: if you release the mouse outside the widget, on_click won't be triggered and the cursor will remain in its old place. This isn't a bug.
  end
  ...
  wgt.on_unfocus = function() return true end
