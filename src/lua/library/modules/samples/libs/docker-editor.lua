...
The reason the two dockers don't currently share code is because of the
...
local function make_room(dlg, space)
  ...
  for edt in dlg:gmatch('Editbox') do
    ...
    if edt.fullscreen then
      edt.x = space.west
      edt.y = space.north + 1
      edt.rows = center_rows
      edt.cols = center_cols
    else
      -- @todo: We should figure out what to do with Editboxes that aren't
      -- "fullscreen". The above lines would have maximized them. Right now
      -- we simply ignore them. This seems to be the sensible thing to do:
      -- even MC doesn't adjust their dimensions when the terminal resizes.
    end
