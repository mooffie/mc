...
- The locks are stored in a dedicated directory (in contrast to each lock being
  ...
  This is intentional: it lets applications like "Recently visited files" show
...
local function lock_files()
  return List(fs.tglob(M.dir .. '/*' .. ext))
end

------------------------------------ lock ------------------------------------

function M.lock(resource_name)
  assert(fs.write(lock_path(resource_name),
    resource_name, "\n",
    os.getpid(), "\n"
  ))
end
...
function M.is_locked(resource_name)
  return fs.file_exists(lock_path(resource_name)) and validate_lock(resource_name)
end
