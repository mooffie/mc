...
--
-- Note: these are in GNOME Terminal's configuration string format.
--
M.palettes = {

  --
  -- 4 palettes copied from GNOME Terminal using:
  --
  ...
  -- Wikipedia (see next) describes the following as "Standard VGA colors".
  ...
  --
  -- The following were imported from Wikipedia's "ANSI escape code" article using this script:
  --     https://gist.github.com/mooffie/c89258dec7ca8f545d8bddda5b26e281
  --

  putty = '#000000000000:#BBBB00000000:#0000BBBB0000:#BBBBBBBB0000:#00000000BBBB:#BBBB0000BBBB:#0000BBBBBBBB:#BBBBBBBBBBBB:#555555555555:#FFFF55555555:#5555FFFF5555:#FFFFFFFF5555:#55555555FFFF:#FFFF5555FFFF:#5555FFFFFFFF:#FFFFFFFFFFFF',
  windows_xp_cmd
        = '#000000000000:#808000000000:#000080800000:#808080800000:#000000008080:#808000008080:#000080808080:#C0C0C0C0C0C0:#808080808080:#FFFF00000000:#0000FFFF0000:#FFFFFFFF0000:#00000000FFFF:#FFFF0000FFFF:#0000FFFFFFFF:#FFFFFFFFFFFF',
  -- This differs a bit from our own 'xterm': in the blue colors (#4, #12)
  -- and darkgray (#8). @todo: figure out why the difference.
  xterm2
        = '#000000000000:#CDCD00000000:#0000CDCD0000:#CDCDCDCD0000:#00000000EEEE:#CDCD0000CDCD:#0000CDCDCDCD:#E5E5E5E5E5E5:#7F7F7F7F7F7F:#FFFF00000000:#0000FFFF0000:#FFFFFFFF0000:#5C5C5C5CFFFF:#FFFF0000FFFF:#0000FFFFFFFF:#FFFFFFFFFFFF',
  -- Mac OS X
  terminal_app
        = '#000000000000:#C2C236362121:#2525BCBC2424:#ADADADAD2727:#49492E2EE1E1:#D3D33838D3D3:#3333BBBBC8C8:#CBCBCCCCCDCD:#818183838383:#FCFC39391F1F:#3131E7E72222:#EAEAECEC2323:#58583333FFFF:#F9F93535F8F8:#1414F0F0F0F0:#E9E9EBEBEBEB',
...
--
-- Converts a terminal color (a number from 0 to 255, above (1 << 24), or -1) to
...
local function html_color(idx)
  ...
  elseif idx >= 0x1000000 then

    -- 16M colors (has the 25'th bit set).
    return ("#%06X"):format(idx - 0x1000000)
...
function M.htmlize_to_file(input, output_filename)
  ...
  assert(fs.write(output_filename, M.template:format(css, source)))
