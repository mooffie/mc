...
-- property. And we could subclass ui.Custom and define the update() method
-- in the conventional way. This is left as an exercise for the reader.)
--
...
local function scrollbar_constructor(dlg)
  ...
  rawset(sb, 'update', function(self, edt)
    ...
    local top, ht = scrollbar_utils.calculate(edt.max_line, edt.rows - (edt.fullscreen and 0 or 2), edt.top_line, self.rows)
    if top then
