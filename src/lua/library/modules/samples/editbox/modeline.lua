...
ui.Editbox.bind("<<load>>", function(edt)
  ...
  local block = edt:sub(1, 1024) .. edt:sub(-1024)
  local is_emacs = false

  local mod = block:p_match { [[(?:\s|^)
                                (?:vi|vim|ex):\s*
                                (?:set?\s+)?
                                ([^\n]*)]], "x" }
  if not mod then
    mod = block:p_match [[-\*-\s*(.*?)\s*-\*-]]
            or block:p_match { [[Local Variables:(.*?)End:]], "s" } -- The manual page insinuates this is case sensitive.
    is_emacs = true
  end
