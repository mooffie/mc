...
local function short_path(path, max)
  ...
  local ellipsis = (tty.is_utf8() and os.getenv('DISPLAY')) and '…' or '<'
  if s:len() > max then
...
-- Query: "does a shot have property XYZ?"
local function has_property(shot, prop)
  return
    (shot.single and shot.single[prop]) or
    (shot.left   and shot.left[prop])   or
    (shot.right  and shot.right[prop])
end

local function render_symbol(shot)
  local is_rich = function() return has_property(shot, 'sort_field') end
  local is_panelized = function() return has_property(shot, 'snapshot_panelized_list') or has_property(shot, 'snapshot_panelized_command') end

  if is_panelized() then
    return 'P'
  elseif is_rich() then
    return '+'
  else
...
local function format_title(shot, width)
  ...
  local room_left = math.max(width - tty.text_width(s), 15)
