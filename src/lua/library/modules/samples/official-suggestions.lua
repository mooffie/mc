...
require('samples.accessories.restore-selection').install()
-- (If something is already typed on the command-line, '&' behaves just like any ordinary key.)
...
-- Type :help on the command-line to see the available commands.
require('samples.colon')
...
-- Make the ENTER key follow, in panelized listings.
-- Comment this out if you don't want this feature.
ui.Panel.bind_if_commandline_empty('enter', function(pnl)
  if not pnl.panelized then  -- If we aren't panelized, let MC handle this key.
    return false
  end
  follow(pnl)
end)
...
-- A frontend for the scanner, via SANE's scanimage(1). If you're using
-- a different software then see our module's documentation for how to
-- customize it.
ui.Panel.bind('C-x M-s', function()
  require('samples.frontends.scanimage').run()
end)
...
ui.Editbox.bind('M-*', function(edt)
  ...
  edt:add_keyword(edt.current_word, tty.style('editor.editbold'), {range='all'})
  edt:redraw()
end)
