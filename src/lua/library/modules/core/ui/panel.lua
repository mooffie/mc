...
-- [info]
--
-- This is just a convenience function that uses @{panelize_by_command}
-- internally. It works by writing the file names to a temporary file
-- and calling @{panelize_by_command} with the `cat` shell command.
--
-- [/info]
--
-- [note]
--
-- A file must exist to be shown in the panel. If you name files that
-- don't exist, they simply won't be shown:
--
...
--
-- This "limitation", of course, pertains to @{panelize_by_command} too.
-- MC needs to @{stat(2)|stat} every file you feed it (in order to show
-- its size, date, etc), so it can only deal with existing files.
--
-- [/note]
--
-- [tip]
...
-- [/tip]
--
-- @function panelize_by_list
-- @args (list)
function ui.Panel.meta:panelize_by_list(list)
  assert_arg_type(1, list, "table")
  local temp_path = fs.temporary_string_file(table.concat(list, "\n"), "\n")
  self:panelize_by_command(("cat %q"):format(temp_path))
  assert(fs.unlink(temp_path))
end
...
-- key is active only when nothing is already typed on the command-line.
--
-- You already recognize this behavior from MC's built-in keys "+" (Select),
-- "-" (Unselect), etc., which aren't effective when the command-line isn't
-- empty. Naturally, if you pick a printable key you'd pick one that doesn't
-- usually start a shell command, and you probably won't find the need to use
-- the modifiers `ALT` or `CTRL`.
--
-- Example:
--
...
-- [info]
--
-- There's nothing "magical" in this function. Its implementation is quite
-- simple:
--
...
-- [/info]
--
-- @function ui.Panel.bind_if_commandline_empty
-- @args (keyseq, fn)
function ui.Panel.bind_if_commandline_empty(keyseq, fn)
  ui.Panel.bind(keyseq, function(pnl)
    if ui.current_widget('Input') and ui.current_widget('Input').text ~= '' then
