...
Info-short: Feel free to ignore this function unless you're an advanced
user.

Sometimes you want to postpone code a bit, till after some other things
happen in the current event-loop iteration.
...
ui.queue() is a very simple function. Its implementation is largely
equivalent to:
...
This trick, of wrapping code in `set_timeout(..., 0)`, is well-known in
the [JavaScript world](http://google.com/search?q=settimeout+0).
...
function ui.queue(fn)
  timer.set_timeout(function()
    -- 'fn' may call mc.edit() or dialog:focus(), which start an event loop.
    -- Timers won't tick there unless we unlock the timer protection here.
    timer.unlock()
    fn()
    tty.refresh()  -- When using timers, we have to refresh the screen ourselves (see explanation in tty module).
  end, 0)
