...
local function choose_style(t)
  ...
  if tty.is_truecolor() then
    color = t.truecolor or t.hicolor or t.color or mono()
  elseif tty.is_hicolor() then
    color = t.hicolor or t.color or mono()
  elseif tty.is_color() then
    color = t.color or mono() or (
                -- If there isn't even "{hi,true}color", most probably it's a spelling error in "color",
                -- so we don't blindly do 'or ""' as in the following "mono" case.
              (t.hicolor or t.truecolor) and "")
  else -- mono
    color = mono() or ""
  end
...
terminal type:
    ...
    local style = tty.style {
      -- Any of these is optional. The one matching the terminal's
      -- capability the most will be selected.
      mono = "reverse",
      color = "yellow, green",
      hicolor = "rgb452, rgb111",
      truecolor = "rgb452, #e16d7e"
    }
    -- 'hicolor' is for 256 color terminals;
    -- 'truecolor' is for 16 million color terminals.
