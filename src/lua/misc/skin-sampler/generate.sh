...
# The desired terminal size (we pass these to stty).
COLS=100
LINES=30
...
[ -z "$MC_SID" ] || die "You cannot run this script from inside MC. Exit MC first."

# Some setups don't enable this by default.
export TERM=xterm-256color

# Set terminal size.
stty cols $COLS rows $LINES
...
for skin in $SKINS_DIR/*.ini; do
  ...
  # The "> /dev/null" indirection is NOT mandatory. Unless we use it, GNOME
  # Terminal sometimes notices we tweaked the screen size and restores it --something
  # we want to avoid.
  # !!!!!! If MC gets stuck, it means an exception dialog is showing. Remove this to see it. !!!!!!
  $MC -S $skin #> /dev/null
