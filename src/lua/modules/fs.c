...
luaFS_push_error_message (lua_State * L, int error_num, const char *filename)
{
     ...
     * string". The locale's encoding may be otherwise. */
    if (filename)
 ...
 *
 * If the file isn't on a local file system, this function creates a copy
 * of it, in some temporary directory, and returns the path for this copy.
 *
 * Info-short: If the file is already on a local file system, this function
 ...
 * original file.
 *
 * Info-short: If the original file is already on the local FS, this
 * function is a no-op.
 *
 ...
 * are resolved.
 *
 * This only works for paths in the local FS, and the path has to point to
 * a file that exists. If these conditions aren't met, **nil** is returned.
 *
...
static const luaMC_constReg fslib_constants[] = {
     ...
     * the second argument to fs.VPath().)
     */
    REGC (VPF_NONE),
    REGC (VPF_RECODE),
    REGC (VPF_STRIP_HOME),
