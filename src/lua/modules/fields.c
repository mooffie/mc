...
To fix this problem we keep a track of who's the "current
field". We also want to know who's the "current panel" because the Lua
programmer might want to pull information out of this object (e.g., the
file's directory).

This tracking task is carried out by set_lua_current_field(). We have to
...
static const char *
render_multiplex (file_entry * fe, int len)
{
...
static int
sort_multiplex (file_entry * a, file_entry * b)
{
    /**
...
l_register_field (lua_State * L)
{
    ...
    static sortfn *sort[] = {
        (sortfn *) sort_name, (sortfn *) sort_vers, (sortfn *) sort_ext,
        (sortfn *) sort_size, (sortfn *) sort_time, (sortfn *) sort_atime,
        (sortfn *) sort_ctime, (sortfn *) sort_inode, (sortfn *) unsorted
    };
    ...
    field.hotkey        = luaL_checkstring(L, 3);   /* sort indicator */
    field.min_size      = luaL_checkint(L, 4);      /* "min_size" is a misnomer actually. It's the _default_ size. */
    ...
    if (has_sort)
    {
        if (lua_type (L, 8) == LUA_TSTRING)
            ...
            field.sort_routine = (sortfn *) sort_multiplex;
