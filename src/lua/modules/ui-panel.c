...
{
    lua_pushstring (L, LUA_TO_PANEL (L, 1)->sort_info.sort_field->id);
    return 1;
}
...
{
    lua_pushinteger (L, LUA_TO_PANEL (L, 1)->count);
    return 1;
}
...
panel_remove_entry (WPanel * panel, int i)
{
    ...
    if (i < 1 || i >= panel->count)
        return;

    g_free (dir.list[i].fname);

    memmove (&dir.list[i], &dir.list[i + 1], sizeof dir.list[0] * (panel->count - i - 1));

    /* @todo: we should also update panel->marked and and panel->total. */

    panel->count--;

    if (panel->selected > i || panel->selected == panel->count)
        panel->selected--;
}
