...
(1) When you close the last open file, conditionally create a stamp. You
    do this with vfs_stamp_create(). (The meaning of "conditionaly" is
    explained later.)

(2) When you open a file, delete the stamp. You do this with
    vfs_rmstamp().

(3) When a path inside your filesystem is invoked, call vfs_stamp() to
    postpone the free'ing of your filesystem a bit. (This simply updates a
    timestamp variable inside the stamp.)
...
only if no directories are open (aka "active") on your filesystem. (If
there are directories open, it means the filesystem is in use, in which
case we don't want to free it.)
